...
public class AuthenticationControllerImpl implements AuthenticationController {
    ...
    public LoginResponseDto login(LoginRequestDto request) {
        if (Objects.isNull(request.getLogin()) || request.getLogin().isEmpty()) {
            throw new TwitterCommandException("Логин не может быть пустым");
        }
        if (request.getLogin().contains(" ")) {
            throw new TwitterCommandException("Логин не может содержать пробелы");
        }

        try {
            User user = userService.getUserByLogin(request.getLogin());
            if (!user.getPassword().equals(/*request.getPassword()*/this.passwordEncoder.encode(request.getPassword()))) {
                throw new TwitterCommandException("Пароль введен неверно");
            }
            ...
            throw new TwitterCommandException("Не найден пользователь с логином: " + request.getLogin());
        }
    }
}
