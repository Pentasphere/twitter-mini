...
public class RegistrationControllerImpl implements RegistrationController {
    ...
    public InfoResponseDto register(RegisterRequestDto request) {
        if (request.getUserType() != 0 && request.getUserType() != 1) {
            throw new TwitterCommandException("Введен неверный тип пользователя.");
        }

        if (Objects.isNull(request.getLogin()) || request.getLogin().isEmpty() || request.getLogin().length() < 5) {
            throw new TwitterCommandException("Логин не может быть пустым или менее 5 символов");
        }

        if (userService.isUserExists(request.getLogin())) {
            throw new TwitterCommandException("Пользователь с таким логином уже существует.");
        }

        if (Objects.isNull(request.getPassword()) || request.getPassword().isEmpty() || request.getPassword().length() < 5) {
            throw new TwitterCommandException("Пароль не может быть пустым или менее 5 символов");
        }

        if (request.getPassword().contains(" ")) {
            throw new TwitterCommandException("Пароль не может содержать пробелы");
        }
        ...
        try {
            ...
            if (UserType.PERSON.equals(userType)) {
                ...
                if (birthDate.isBefore(LocalDate.now().minusYears(100))) {
                    throw new TwitterCommandException("Дата рождения не может быть настолько в прошлом");
                }
    ...
    private void validateField(String field, String errorMessage) {
        if (Objects.isNull(field) || field.trim().isEmpty()) {
            throw new TwitterCommandException(errorMessage);
        }
    }

    private LocalDate validateDate(String date, String dateType) {
        if (Objects.isNull(date) || date.trim().isEmpty()) {
            throw new TwitterCommandException("Дата " + dateType + " не может быть пустой");
        }
        String regex = "^\\d{4}-\\d{2}-\\d{2}$";
        if (!date.matches(regex)) {
            throw new TwitterCommandException("Неверно введена дата " + dateType);
        }
        DateTimeFormatter formatter = DateTimeFormatter.ofPattern("yyyy-MM-dd");
        LocalDate parseDate = LocalDate.parse(date, formatter);
        if (parseDate.isAfter(LocalDate.now())) {
            throw new TwitterCommandException("Дата " + dateType + " не может быть в будущем");
        }
        return parseDate;
    }
}
